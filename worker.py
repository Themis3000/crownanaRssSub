import datetime
from typing import List
from db import QueryManager
from db.sqlc.models import FeedHistory
from db.sqlc.queries import find_notify_mark_updating_subsRow
from rss import get_posts
from functools import lru_cache
from email_service import email_serv
import time
from utils import store_posts
from async_tools import sync_list


async def do_work():
    """Runs forever. Handles doing whatever tasks need to be done."""
    while True:
        # Update a single feed if one is ready.
        did_feed_job = await do_feed_job()

        # Do up to 200 mail jobs at a time. If there's more it can be completed later.
        did_mail_job = await do_mail_jobs(limit=200)

        if not did_feed_job and not did_mail_job:
            # There is no job to do. Wait 30 seconds before checking again...
            time.sleep(30)
            continue


async def do_feed_job() -> bool:
    """Tries to find a feed in need of a refresh. Returns if feed was refreshed"""
    async with QueryManager() as q:
        feed_job = await q.get_feed_to_run()

        if feed_job is None:
            return False

        try:
            posts = get_posts(rss_url=feed_job.rss_url, last_id=feed_job.unique_id, last_date=feed_job.post_date)
        except Exception:
            await q.feed_set_last_fail_now(feed_id=feed_job.feed_id)
            return True

        await q.feed_set_last_check_now(feed_id=feed_job.feed_id)

        if len(posts.rss_posts) == 0:
            return True

        await store_posts(q=q, feed_id=feed_job.feed_id, updates=posts)
        await q.mark_feed_updates(feed_id=feed_job.feed_id)
        return True


async def do_mail_jobs(limit=100) -> bool:
    """Tries to find users with a notification due and executes the notification.
    Returns if job was found and completed"""
    async with QueryManager() as q:
        subs = await sync_list(q.find_notify_mark_updating_subs(limit=limit))
        if len(subs) == 0:
            return False

    @lru_cache(maxsize=128)
    async def caching_get_feed_history_since_id(feed_id: int, history_id: int, history_limit: int) -> List[FeedHistory]:
        async with QueryManager() as q:
            return await sync_list(
                q.get_feed_history_since_id(feed_id=feed_id, history_id=history_id, limit=history_limit))

    for sub in subs:
        # If coming up on the 5 minute limit to complete the batch, stop early to ensure no double emails.
        if sub.last_process_update + datetime.timedelta(minutes=4, seconds=30) < datetime.datetime.now():
            break

        post_history = await caching_get_feed_history_since_id(feed_id=sub.feed_id,
                                                               history_id=sub.last_post_notify,
                                                               history_limit=20)

        await send_mail_notification(sub=sub, post_history=post_history)

    return True


async def send_mail_notification(sub: find_notify_mark_updating_subsRow, post_history: List[FeedHistory]):
    # The length of new_posts should never ever be 0.
    # But just in case, mark as notified anyway and move on.
    if len(post_history) == 0:
        async with QueryManager() as q:
            await q.mark_subscriber_notified(subscriber_id=sub.subscriber_id, last_post_notify=sub.last_post_notify)
        return

    # I'm marking notified before sending the email. If it fails it's not that big of a deal.
    # I'd rather things fail open in this way.
    async with QueryManager() as q:
        await q.mark_subscriber_notified(subscriber_id=sub.subscriber_id, last_post_notify=post_history[-1].history_id)

    email_serv.notify_update(to_addr=sub.email, posts=post_history, blog_name=sub.feed_name)
